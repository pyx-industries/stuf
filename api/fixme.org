...
* Active Tasks
** NEXT Stabilize Integration Test Suite (P0)
...
*** Completion Criteria
- [ ] The `integration_client` fixture in `api/tests/integration/conftest.py` is fixed and correctly injects all mocks.
- [ ] All `ERROR`s during test teardown are resolved.
- [ ] All previously failing integration tests in `test_files_api.py` (upload, list, download) are passing.
- [ ] The test suite is stable and provides a reliable baseline for further refactoring.

** TODO Refactor MinioClient for Dependency Injection (P1)
...
*** Completion Criteria
- [ ] The `app.dependency_overrides[MinioClient]` line in `api/main.py` is removed.
- [ ] `api/routers/files.py` endpoints continue to correctly use `Depends(MinioClient)`.
- [ ] Integration and unit test fixtures cleanly override the `MinioClient` dependency.
- [ ] All unit and integration tests pass reliably.
- [ ] The application remains functional when run normally (e.g., via `uvicorn` or `docker-compose`), with the real `MinioClient` being created by FastAPI's dependency injection system.
