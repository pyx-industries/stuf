...
* Active Tasks
** NEXT Refactor MinioClient for Dependency Injection (P1)
This is a critical refactoring to address the fundamental design problem of the `MinioClient` singleton being instantiated at module import time in `api/storage/minio.py`. This current approach leads to network calls during test collection (causing "Connection refused" warnings) and makes reliable mocking in integration tests extremely difficult and brittle.

The "holy grail" solution is to transition `MinioClient` from a global singleton to a dependency that can be injected, leveraging FastAPI's `Depends` mechanism. This will:
- Prevent unwanted network calls during module import.
- Enable straightforward and robust mocking for unit and integration tests.
- Align the `MinioClient`'s usage with best practices for testability and maintainability.

*** Files to create/modify
- `api/storage/minio.py`: Modify the `MinioClient` class to remove its module-level instantiation. It should simply be a class. The `_ensure_bucket_exists` call in `__init__` should remain, but should only be executed when a *real* client is desired.
- `api/routers/files.py`: Update API endpoints to receive an instance of `MinioClient` via `FastAPI.Depends()`.
- `api/main.py`: Add an application-level dependency override for `MinioClient` when running the application directly, or for local development.
- `api/tests/integration/conftest.py`: Simplify the MinIO mocking significantly by using FastAPI's `app.dependency_overrides` to inject a `MagicMock` of `MinioClient`. Remove the complex `sys.modules` manipulation and class/init patching.
- `api/tests/unit/conftest.py`: Adjust `mock_minio_client` fixture to override the dependency in FastAPI instead of patching the singleton.

*** Completion Criteria
- [ ] The `minio_client = MinioClient()` line in `api/storage/minio.py` is removed, making `MinioClient` a regular class.
- [ ] `api/routers/files.py` endpoints correctly use dependency injection for `MinioClient`.
- [ ] `api/tests/integration/conftest.py` uses `app.dependency_overrides` for `MinioClient` mocking.
- [ ] `api/tests/unit/conftest.py` uses `app.dependency_overrides` for `MinioClient` mocking.
- [ ] All unit and integration tests pass reliably without "Connection refused" warnings.
- [ ] The application remains functional when run normally (e.g., via `uvicorn` or `docker-compose`).

** TODO Refactor E2E Test Setup using Pytest-Docker (P2)
