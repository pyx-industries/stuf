...
class TestAuthFlowWithScreenshots:
    ...
    def test_complete_auth_flow_with_numbered_screenshots(self, page: Page):
        ...
        dashboard.take_screenshot("application-loaded", scenario_name, 1, "Given I navigate to the STUF application", "direct")
        ...
        dashboard.take_screenshot("ready-to-click-login", scenario_name, 2, "When I click the login button", "direct")
        
        # Step 3: Click login button and wait for redirect
        login_button.click()
        page.wait_for_timeout(1000)  # Allow navigation to start
        dashboard.take_screenshot("redirecting-to-idp", scenario_name, 3, "And I am redirected to the IDP login page", "direct")
        
        # Step 4: At Keycloak login page
        login_page = LoginPage(page)
        login_page.wait_for_login_form(timeout=15000)
        login_page.assert_login_form_visible()
        login_page.take_screenshot("at-idp-login-form", scenario_name, 4, "And I enter valid admin credentials", "direct")
        
        # Step 5: Enter credentials
        login_page.fill_username("admin@example.com")
        login_page.fill_password("password")
        login_page.take_screenshot("credentials-entered", scenario_name, 5, "And I click the IDP login button", "direct")
        ...
        dashboard.take_screenshot("back-at-spa-authenticated", scenario_name, 6, "Then I should be redirected back to the application", "direct")
        
        # Step 8: Verify dashboard is visible and user is logged in
        dashboard.take_screenshot("dashboard-visible", scenario_name, 7, "And I should see the dashboard", "direct")
        ...
        try:
            ...
            dashboard.take_screenshot("session-persisted-after-reload", scenario_name, 8, "And I should be logged in as an authenticated user", "direct")
        except:
            dashboard.take_screenshot("session-lost-after-reload", scenario_name, 8, "And I should be logged in as an authenticated user", "direct")
