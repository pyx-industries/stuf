...
class LoginPage(BasePage):
    ...
    def wait_for_login_form(self, timeout: int = 30000) -> None:
        ...
        self.page.wait_for_url(f"{self.keycloak_url}/realms/stuf/protocol/openid-connect/auth*", timeout=timeout)
    ...
    def login(self, username: str, password: str) -> None:
        ...
        try:
            ...
            if "localhost:3100" not in current_url:
                raise RuntimeError(f"Login failed - not redirected to SPA. Current URL: {current_url}")
    ...
    def assert_login_form_visible(self) -> None:
        ...
        self.assert_element_visible(self.USERNAME_INPUT, "Username input should be visible")
        self.assert_element_visible(self.PASSWORD_INPUT, "Password input should be visible")
    ...
    def assert_error_message_visible(self, expected_message: str = None) -> None:
        ...
        self.assert_element_visible(self.ERROR_MESSAGE, "Error message should be visible")
        if expected_message:
    ...
    def assert_on_keycloak_page(self) -> None:
        ...
        current_url = self.get_current_url()
        assert self.keycloak_url in current_url, f"Should be on Keycloak page, but URL is: {current_url}"
    ...
    def attempt_invalid_login(self, username: str = "invalid", password: str = "wrong") -> None:
