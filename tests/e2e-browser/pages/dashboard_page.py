...
class DashboardPage(BasePage):
    ...
    LOGOUT_BUTTON = '[data-testid="logout"], button:has-text("Logout"), button:has-text("Sign out")'
    UPLOAD_BUTTON = '[data-testid="upload"], button:has-text("Upload"), input[type="file"]'
    ...
    def get_file_list_items(self) -> List[str]:
        ...
        return [file_elements.nth(i).text_content() or "" for i in range(file_elements.count())]
    ...
    def assert_success_message(self, expected_message: str = None) -> None:
        ...
        self.assert_element_visible(self.SUCCESS_MESSAGE, "Success message should be visible")
        if expected_message:
    ...
    def search_files(self, search_term: str) -> None:
        ...
        search_selector = 'input[type="search"], input[placeholder*="search"], [data-testid="search"]'
    ...
    def assert_on_dashboard(self) -> None:
        ...
        assert "localhost:3100" in current_url, f"Should be on dashboard, but URL is: {current_url}"
