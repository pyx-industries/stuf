...
def authenticated_page(page):
    ...
    try:
        ...
        if login_button.is_visible():
            ...
            page.wait_for_url(f"{KEYCLOAK_URL}/realms/stuf/protocol/openid-connect/auth*", timeout=10000)
        else:
    ...
    try:
        ...
        for attempt in range(max_attempts):
            ...
            raise RuntimeError(f"Not redirected to SPA after login. Current URL: {page.url}")
    ...
    try:
        ...
        if auth_required.is_visible():
            raise RuntimeError("Authentication failed - still seeing auth required message")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(f"⏳ {name} not ready yet, retrying in {retry_delay}s... ({attempt + 1}/{max_retries})")
                    time.sleep(retry_delay)
                else:
                    raise RuntimeError(f"❌ {name} failed to become ready after {max_retries} attempts: {e}")
        else:
...
def pytest_bdd_step_error(request, feature, scenario, step, step_func, step_func_args, exception):
    ...
    if "page" in step_func_args:
        ...
        try:
            screenshot_path = Path(__file__).parent / "reports" / "screenshots" / f"step-failure-{scenario.name.replace(' ', '-')}.png"
