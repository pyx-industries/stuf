...
*** A Note on OIDC Clients
In OIDC terminology, both the SPA and the API are considered "clients," but they serve different roles. The SPA is a =public client= running in an untrusted browser, responsible for initiating the user's login. The API is a =confidential client= (acting as a "Resource Server") that can hold a secret and is responsible for validating tokens and protecting data. This separation is a security best practice.
...
- *Required Scopes*: =openid stuf:access=
